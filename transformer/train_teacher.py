...
class TeacherNet(nn.Module):
    ...
    def forward(self, graph):
        ...
        # embed
        src_embed, src_pos = self.src_embed(graph.src[0]).to(devices[0]), self.pos_enc(graph.src[1]).to(devices[0])
...
def main():
    ...
    # Create model
    model = make_model(V, V, N=N, dim_model=128, dim_ff=128, h=8).to(devices[0])
    # Sharing weights between Encoder & Decoder
    model.src_embed.lut.weight = model.tgt_embed.lut.weight.to(devices[0])
    model.generator.proj.weight = model.tgt_embed.lut.weight.to(devices[0])
    ...
    loss_compute = partial(SimpleLossCompute, criterion, 1)
    ...
    for epoch in range(100):
        train_iter = dataset(graph_pool, mode='train', batch_size=batch_size, device=devices[0])
        valid_iter = dataset(graph_pool, mode='valid', batch_size=batch_size, device=devices[0])
        print('Epoch: {} Training...'.format(epoch))
        model.train(True).to(devices[0])
        run_epoch(train_iter, model, loss_compute(model_opt), is_train=True)
        print('Epoch: {} Evaluating...'.format(epoch))
        model.att_weight_map = None
        model.eval().to(devices[0])
        run_epoch(valid_iter, model, loss_compute(None), is_train=False)
        att_maps.append(model.att_weight_map)
        teacher_history.append((loss_compute.avg_loss, loss_compute.accuracy))
    ...
    th.save(model.state_dict(), "teacher.pt")
